...
def value_metrics(
    ...
    *,
    ...
    explained_variance = (
        ...
        else 1.0 - (float(torch.var(residual, unbiased=False).item()) / target_variance)
    )
    correlation = None
    if prediction_variance > 0.0 and target_variance > 0.0:
        covariance = float(
            torch.mean((prediction - prediction.mean()) * (target - target.mean())).item()
        )
...
def gameplay_metrics(rollout: RolloutBatch) -> tuple[GameplayMetricSlice, ...]:
    ...
    slices = [GameplayMetricSlice("all", "all", _gameplay_aggregate(records))]
    cells = sorted({(record[0], record[1]) for record in records})
    for ruleset_name, player_count in cells:
        selected = [
            record for record in records if record[0] == ruleset_name and record[1] == player_count
        ]
        slices.append(
            GameplayMetricSlice(
                "cell",
...
def _gameplay_aggregate(
    records: Sequence[tuple[str, int, int, float, tuple[RolloutTransition, ...]]],
) -> GameplayMetrics:
    transitions = tuple(transition for record in records for transition in record[4])
    positive_bids = tuple(
        transition.action for transition in transitions if 1 <= transition.action <= 100
    )
    return GameplayMetrics(
        ...
        pass_rate=sum(transition.action == 0 for transition in transitions) / len(transitions),
        mean_positive_bid=(sum(positive_bids) / len(positive_bids) if positive_bids else None),
        illegal_actions=sum(
