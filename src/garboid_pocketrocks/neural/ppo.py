    ...
    def update(
        ...
        *,
        ...
        for epoch_index in range(self.config.epochs):
            ...
            for start in range(0, transition_count, self.config.minibatch_size):
                ...
                if not batch.action_mask.gather(1, actions.unsqueeze(1)).all().item():
                    raise PPOError(
                        "rollout contains an action illegal under its stored mask"
                    )
                ...
                if not torch.isfinite(pre_clip).item() or not math.isfinite(
                    post_clip
                ):
                    raise PPOError(
                        "gradient clipping produced a nonfinite norm"
                    )
                self.optimizer.step()
                _ensure_model_finite(self.model)
                ...
                approximate_kl_sum += float(
                    (old_log_probability - new_log_probability)
                    .detach()
                    .sum()
                    .item()
                )
                clip_count += int(
                    (
                        torch.abs(loss.ratio.detach() - 1.0)
        ...
        value_slices = stratified_value_metrics(
            ...
            targets.returns,
            ruleset_names=tuple(
                f"live-{chr(ord('A') + int(index))}"
                for index in packed.chart_indices
            ),
