...
def collect_self_play(
    ...
    *,
    ...
    def fill_active() -> None:
        ...
        while len(active) < active_games and remaining_index < len(collected_plans):
    ...
    cell_counts = Counter(
        (episode.plan.ruleset_name, episode.plan.player_count) for episode in completed
    )
...
def _validate_collection(
    ...
    *,
    ...
    if not isinstance(active_games, int) or isinstance(active_games, bool) or active_games <= 0:
    ...
    for identity in required_identities:
        ...
        if not _devices_match(model_device, device):
            raise CollectorError(f"policy {identity!r} is on {model_device}, expected {device}")
