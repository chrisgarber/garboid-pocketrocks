...
class PackedRollout:
    ...
    def from_batch(cls, rollout: RolloutBatch) -> PackedRollout:
        ...
        for episode_index, trajectory in trajectories:
            ...
            if (
                trajectory[-1].terminated is not True
                or any(item.truncated for item in trajectory)
            ):
        ...
        packed = cls(
            ...
            private_hand_ids=np.stack(
                [item.private_hand_ids for item in observations]
            ),
            ...
            history_numeric=np.stack(
                [item.history_numeric for item in observations]
            ),
            ...
            chart_indices=np.asarray(
                [
                    ord(item.metadata.ruleset_name[-1]) - ord("A")
    ...
    def batch(
        ...
        return NeuralBatch(
            ...
            global_numeric=torch.as_tensor(
                self.global_numeric[indices], device=device
            ),
            objective_bits=torch.as_tensor(
                self.objective_bits[indices], device=device
            ),
            ...
            private_hand_ids=torch.as_tensor(
                self.private_hand_ids[indices], device=device
            ),
            ...
            history_numeric=torch.as_tensor(
                self.history_numeric[indices], device=device
            ),
            history_valid=torch.as_tensor(
                self.history_valid[indices], device=device
            ),
...
def _phase_bucket(transition: RolloutTransition) -> int:
    valid_ids = transition.observation.history_ids[
