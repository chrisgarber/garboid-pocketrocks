...
class SelfPlayGame:
    ...
    def __init__(
        ...
        *,
        ...
        self._pending_rewards = {
            seat: RewardBreakdown() for seat in range(plan.player_count)
        }
    ...
    def apply(self, responses: Sequence[PolicyResponse]) -> None:
        ...
        expected_by_seat = {
            request.seat: request for request in self._pending_requests
        }
        response_by_seat = {response.seat: response for response in supplied}
        if (
            len(response_by_seat) != len(supplied)
            or set(response_by_seat) != set(expected_by_seat)
        ):
