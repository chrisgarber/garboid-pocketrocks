...
def collect_self_play_parallel(
    ...
    *,
    ...
    shards = tuple(
        tuple(collected_plans[index::worker_count])
    ...
    try:
        while open_connections:
            ...
            inference_messages: list[
                tuple[Connection, WorkerInferenceBatch]
            ] = []
    ...
    cells = Counter(
        (item.plan.ruleset_name, item.plan.player_count)
    ...
    metrics = CollectorMetrics(
        ...
        cell_games=tuple(
            (ruleset, count, games)
            for (ruleset, count), games in sorted(cells.items())
        ),
