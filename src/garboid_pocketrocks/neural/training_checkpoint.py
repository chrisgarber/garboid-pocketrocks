    ...
    def __post_init__(self) -> None:
        ...
        if any(
            ...
            raise TrainingCheckpointError(
                "training progress counters must be nonnegative integers"
            )
        if any(
            not ruleset
            or player_count not in (3, 4, 5)
            or games < 0
    ...
    *,
    ...
    try:
        ...
        if isinstance(error, TrainingCheckpointError):
            raise
        raise TrainingCheckpointError(
            f"training checkpoint save failed: {error}"
        ) from error
...
def load_training_checkpoint(
    ...
    *,
    ...
    try:
        ...
        metrics_value: object = json.loads(
            (path / "metrics.json").read_text(encoding="utf-8")
        )
    except (OSError, RuntimeError, ValueError, json.JSONDecodeError) as error:
        raise TrainingCheckpointError(
            "training checkpoint payload could not be loaded"
        ) from error
...
def _run_config(value: object) -> TrainingRunConfig:
    ...
    return TrainingRunConfig(
        ...
        max_wall_seconds=_optional_number(
            payload["max_wall_seconds"], "max_wall_seconds"
        ),
