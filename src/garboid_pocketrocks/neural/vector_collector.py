...
def _collect_engine_batch(
    ...
    *,
    ...
    engine = BatchSimEngine.start(
        ...
        value_charts=tuple(
            plan.ruleset_name.removeprefix("live-") for plan in plans
        ),
    )
        ...
        histories.append(
            [
                PublicGameSetup(
                    ...
                    objective_ids=tuple(
                        int(value)
                        for value in engine.objective_ids[row]
                        if value > 0
                    ),
                )
            ]
    ...
    pending_rewards = [
        [RewardBreakdown() for _ in range(player_count)]
        for _ in plans
    ]
    decision_counts = [
    ...
    while True:
        ...
        response_by_key = {
            (response.episode_index, response.seat): response
            for response in bid_responses
        }
        request_by_key = {
            (request.episode_index, request.seat): request
        ...
        for row_value in active_rows:
            ...
            histories[row].append(
                PublicAuctionResolved(
                    ...
                    bids_by_seat=tuple(
                        int(value) for value in outcome.effective_bids[row]
                    ),
                )
            )
        ...
        reveal_by_key = {
            (response.episode_index, response.seat): response
            for response in reveal_responses
        }
        ...
        for row_value in choice_rows:
            ...
            opened[(row, seat)] = _OpenTransition(
                request=reveal_request_by_key[
                    (plans[row].episode_index, seat)
                ],
            ...
            reveal_indices[row] = (
                ...
                else response.action - encoder_config.max_bid - 1
            )
...
def _potential(engine: BatchSimEngine) -> np.ndarray:
    objective_payouts = np.asarray(
        [
            OBJECTIVE_PAYOUTS.get(objective_id, 0)
            for objective_id in range(1, 31)
        ],
        dtype=np.int16,
    )
    objectives = np.sum(
        engine.owned_objectives.astype(np.int16)
        * objective_payouts[None, None, :],
        axis=2,
    )
    return (
        engine.cash
        + engine.investment_values
        - engine.loan_principal
        + objectives
    ).astype(np.int32)
...
def _apply_resolve_rewards(
    ...
    for row_value in active_rows:
        ...
        objective_claimed = bool(
            np.any(
                engine.owned_objectives[row, winner]
                & ~objectives_before[row, winner]
            )
        )
        ...
        for seat in range(engine.player_count):
            accounting = config.accounting_weight * (
                (
                    int(current_potential[row, seat])
                    - int(previous_potential[row, seat])
                )
                / starting_cash
            )
            pending_rewards[row][seat] = _add_breakdowns(
...
def _apply_terminal_rewards(
    ...
    for row, result in enumerate(results):
        first_place = tuple(
            score for score in result.scores if score.rank == 1
        )
        for score in result.scores:
            placement = (
                config.win_bonus / len(first_place)
                if score.rank == 1
                else 0.0
            )
            ...
            terminal_resource = config.accounting_weight * (
                (
                    score.final_money
                    - int(previous_potential[row, score.seat])
                )
                / starting_cash
            )
            ...
            SessionResult(
                scores=tuple(
                    SessionScore(
                        ...
                        rank=1 + sum(
                            other > totals[seat] for other in totals
                        ),
                    )
                    for seat in range(engine.player_count)
                ),
                rows=rows,
                ranking=tuple(
                    int(seat) for seat in rankings[row]
                ),
            )
        )
    return tuple(output)
        ...
        RolloutTransition(
            ...
            metadata=RolloutMetadata(
                ...
                opponent_names=tuple(
                    opponent.identity
                    for opponent_seat, opponent in enumerate(
                        plan.seat_policies
                    )
