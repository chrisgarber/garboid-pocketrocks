...
class SelfPlayEpisodePlan:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            len(self.seat_sampling_seeds) != self.player_count
            or any(not _is_seed(seed) for seed in self.seat_sampling_seeds)
        ):
            ...
            for player_count in (3, 4, 5):
                ...
                plans.append(
                    SelfPlayEpisodePlan(
                        ...
                        seat_policies=(
                            SeatPolicy(identity=policy_identity, trainable=True),
                        )
                        * player_count,
                    )
                )
...
def decision_seed(
    ...
    if (
        ...
        or not 0 <= seat < plan.player_count
    ):
...
def _derive_seed(root_seed: int, namespace: str, *indices: int) -> int:
    ...
    canonical = ":".join(
        (str(root_seed), namespace, *(str(index) for index in indices))
    )
