...
def _components() -> tuple[
    ...
    loss = torch.stack(
        tuple(parameter.square().mean() for parameter in model.parameters())
    ).sum()
    loss.backward()
    optimizer.step()
...
def _save(path: Path) -> Path:
    ...
    return save_training_checkpoint(
        ...
        manifest=manifest,
        generator_states={
            "policy": torch.Generator(device="cpu").manual_seed(7).get_state()
        },
...
def test_update_boundary_resume_matches_uninterrupted_optimizer_step(
    ...
    assert parameter_digest(model.state_dict()) == parameter_digest(
