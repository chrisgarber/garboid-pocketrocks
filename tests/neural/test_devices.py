...
def test_calibration_candidates_measure_vector_engine_batch_sizes() -> None:
    ...
    assert {candidate.workers for candidate in candidates if candidate.device == "cpu"} == {1, 8}
    assert {candidate.workers for candidate in candidates if candidate.device == "mps"} == {1}
    assert {candidate.active_games_per_worker for candidate in candidates} == {64, 128}
    assert {candidate.max_inference_batch for candidate in candidates} == {1024}
